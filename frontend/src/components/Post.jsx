...
function Post({ id, author, like, comment, description, image, createdAt }) {
  ...
  let [isDeleted, setIsDeleted] = useState(false) // ⬅️ Track deleted state
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setIsDeleted(true); // instantly hide from UI
    } catch (error) {
  ...
  // ⬅️ If deleted, don't render
  if (isDeleted) return null;

  return (
    <div className="w-full min-h-[200px] flex flex-col gap-[10px] bg-white rounded-lg shadow-lg p-[20px] relative">
      ...
      {userData._id === author._id && (
        <button
          ...
          className="absolute bottom-[15px] right-[20px] bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-[14px] font-semibold transition-all duration-200"
        >
          Delete
        </button>
      )}
